...
def show(items,TotalPrice,productCtr,cusDetails):
    ...
    # Loop to print product details one by one from items list
...
def addProduct(items,TotalPrice,productCtr):
    ...
    # qty IP check (mentioned in Day 1)
    while(True):
    ...
    # MRP input check (Mentioned in Day 1)
    while(True):
...
def delete(items,productCtr,TotalPrice,productId):
    productCtr-=1
    # To traverse the items list to find the particular productID and remove it by fixing total price
    for i in items:
        if i[0]==productId:
            TotalPrice-=(i[2]*i[3])
            items.remove(i)
...
# 2.1 User option loop: based on choices user will get redirected to particular funtion call
while(True):
    choice=input("choose\n0: to add product\n1: to show invoice\n2: to remove product\n")
    if choice in ['0','1','2']:
        choice=int(choice)
